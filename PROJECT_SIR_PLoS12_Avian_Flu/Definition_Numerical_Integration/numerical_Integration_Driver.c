                                  ...
                                  int j, double * Time_Current )
{
  ...
  int MODEL_VARIABLES = Table->MODEL_VARIABLES;
  Parameter_Model * P = Table->P;

  gsl_odeiv_step * s 
         = gsl_odeiv_step_alloc (T, MODEL_VARIABLES);
  gsl_odeiv_control * c 
         = gsl_odeiv_control_y_new (GSL_ODE_ERR_0, GSL_ODE_ERR_1);
  gsl_odeiv_evolve * e 
         = gsl_odeiv_evolve_alloc (MODEL_VARIABLES);
     
  gsl_odeiv_system sys = {function, jacobian, MODEL_VARIABLES, P};
